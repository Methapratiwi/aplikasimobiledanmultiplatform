...
export class CategoryProvider { 
  ...
  public get(id: number){     return this.databaseProvider.getDB().then((db: SQLiteObject) => { 
      let sql = 'SELECT * FROM categories WHERE id=?';       let params = [id]; 
...
{         if(data.rows.length > 0){           
  ...
  return category; 
  ...
  }    public getAll(){     return this.databaseProvider.getDB().then((db: SQLiteObject) => { 
      let sql = "SELECT * FROM categories"; 

      return db.executeSql(sql, []).then((data: any) => { 
        //console.log("Jumlah kategori = ", data.rows.length); 
        if(data.rows.length > 0){           let categories: Category[] = []; 
          for (var i = 0; i < data.rows.length; i++) {             let category = new Category();             
    ...
    .catch((e) => console.error(e)); 
  }    public remove(id: number){     return this.databaseProvider.getDB().then((db: SQLiteObject) => { 
      let sql = 'DELETE FROM categories WHERE id=?';       let params = [id]; 
      return db.executeSql(sql, params) 
