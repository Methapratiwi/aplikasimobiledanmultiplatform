...
{ 
    ...
    return this.databaseProvider.getDB().then((db: SQLiteObject) => { 
      let sql = 'SELECT p.*, c.name as category_name FROM products p INNER JOIN favorite f on p.id = f.product_id INNER JOIN categories c on p.category_id = c.id';       let params: any[] = [];       if(name){ 
...
{         if(data.rows.length > 0){           
  ...
  public removeFavorite(id: number){     return this.databaseProvider.getDB().then((db: SQLiteObject) => { 
