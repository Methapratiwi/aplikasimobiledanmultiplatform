...
export class MenuPage { 
  ...
  ionViewDidLoad() { 
    console.log('ionViewDidLoad MenuPage');     this.nav.push(HomePage, {menuAccount: 
this.userAccount}); 
  }  
  initializeApp() { 
...
this.userAccount}); 
  ...
  logout(){ 
    let confirm = this.alerCtrl.create({       title: 'Confirm Logout', 
      message: 'Do you really to logout?',       buttons: [         { 
          text: 'No',           handler: () => { 
            console.log('No clicked'); 
          } 
        },         { 
          text: 'Yes',           handler: () => { 
            console.log('Yes clicked');             this.navCtrl.setRoot(LoginPage); 
          } 
        } 
      ]     }); 
    confirm.present() 
